...
public class DailyRewardMenus extends GUIUtils {
    ...
    public static void mainMenu(Player player) {
        ...
        if (playerData.getClaimedDailyRewardsRank1At() + SECONDS_BETWEEN_CLAIMS <= Instant.now().getEpochSecond()) {
            ...
            c.setItem(2, c.createButton(Material.DIAMOND, "&bClaim Reward &e(Warrior)", List.of("&fYou can claim this reward every", "&f24 hours to get random crate keys!", "",
        ...
        if (playerData.getClaimedDailyRewardsAt() + SECONDS_BETWEEN_CLAIMS <= Instant.now().getEpochSecond()) {
            ...
            c.setItem(4, c.createButton(Material.NETHER_STAR, "&bClaim Reward &a(Member)", List.of("&fYou can claim this reward every", "&f24 hours to get random crate keys!", "",
                    "&cClaim this again in: " + PrisonUtils.formatTime((playerData.getClaimedDailyRewardsAt() + SECONDS_BETWEEN_CLAIMS - Instant.now().getEpochSecond()) * 1000))));
        ...
        if (playerData.getClaimedDailyRewardsRank2At() + SECONDS_BETWEEN_CLAIMS <= Instant.now().getEpochSecond()) {
            ...
            c.setItem(6, c.createButton(Material.AMETHYST_SHARD, "&bClaim Reward &d(Static)", List.of("&fYou can claim this reward every", "&f24 hours to get random crate keys!", "",
