...
public class CustomEnchants {
    ...
    public static long getEnchantLevel(ItemStack item, String enchant) {
        ...
        if(item.getItemMeta().getPersistentDataContainer().has(new NamespacedKey(StaticPrisons.getInstance(), enchant), PersistentDataType.LONG)) {
            return item.getItemMeta().getPersistentDataContainer().get(new NamespacedKey(StaticPrisons.getInstance(), enchant), PersistentDataType.LONG);
        }
        return 0;
    }
    ...
    public static void setEnchantLevel(ItemStack item, String enchant, long level) {
        ...
        meta.getPersistentDataContainer().set(new NamespacedKey(StaticPrisons.getInstance(), enchant), PersistentDataType.LONG, level);
    ...
    public static void updateLore(ItemStack item) {
        ...
        if (meta.getPersistentDataContainer().has(new NamespacedKey(StaticPrisons.getInstance(), "customEnchantLore"), PersistentDataType.STRING)) {
            String[] enchantLore = meta.getPersistentDataContainer().get(new NamespacedKey(StaticPrisons.getInstance(), "customEnchantLore"), PersistentDataType.STRING).split("\\$\\+\\$\\+\\$");
        ...
        meta.getPersistentDataContainer().set(new NamespacedKey(StaticPrisons.getInstance(), "customEnchantLore"), PersistentDataType.STRING, customEnchantLore.toString());
