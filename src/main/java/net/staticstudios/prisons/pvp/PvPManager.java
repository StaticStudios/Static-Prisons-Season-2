...
public class PvPManager {
    ...
    public static void init() {
        ...
        Bukkit.getScheduler().runTaskTimer(StaticPrisons.getInstance(), () -> {
            for (Map.Entry<Player, Long> entry : playerWarpOutIn.entrySet()) {
                if (entry.getValue() > 0 && entry.getValue() <= 5) entry.getKey().sendMessage(PREFIX + ChatColor.GREEN + "Warping to spawn in " + entry.getValue() + "...");
    ...
    static class Listener implements org.bukkit.event.Listener {
        ...
        void onHit(EntityDamageByEntityEvent e) {
            if (!(e.getDamager() instanceof Player attacker && e.getEntity() instanceof Player gotHit)) return; //Ensure a player hit a player
