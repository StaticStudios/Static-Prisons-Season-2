...
public class PlayerRanksListener implements Listener {
    ...
    void onBlockBreakProcess(BlockBreakProcessEvent e) {
        ...
        switch (blockBreak.getPlayerData().getPlayerRank()) {
            ...
            case "static" ->
                    blockBreak.stats().setTokenMultiplier(blockBreak.stats().getTokenMultiplier() + 0.1d); //+10%
            case "staticp" ->
                    blockBreak.stats().setTokenMultiplier(blockBreak.stats().getTokenMultiplier() + 0.15d); //+15%
        }
