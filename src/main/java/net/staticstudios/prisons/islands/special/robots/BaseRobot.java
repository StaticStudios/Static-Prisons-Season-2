package net.staticstudios.prisons.islands.special.robots;
...
public abstract class BaseRobot {
    ...
    public static boolean entityClicked(PlayerInteractEntityEvent e) {
        ...
        if (!e.getRightClicked().getPersistentDataContainer().has(new NamespacedKey(StaticPrisons.getInstance(), "isRobot"), PersistentDataType.INTEGER)) return false;
    ...
    public static void spawnRobot(Location loc, String type, Color armorColor, String name) {
        ...
        zombie.getPersistentDataContainer().set(new NamespacedKey(StaticPrisons.getInstance(), "isRobot"), PersistentDataType.INTEGER, 1);

        //Default stats
        zombie.getPersistentDataContainer().set(new NamespacedKey(StaticPrisons.getInstance(), "type"), PersistentDataType.STRING, type);
        zombie.getPersistentDataContainer().set(new NamespacedKey(StaticPrisons.getInstance(), "level"), PersistentDataType.INTEGER, 1);
        zombie.getPersistentDataContainer().set(new NamespacedKey(StaticPrisons.getInstance(), "oil"), PersistentDataType.DOUBLE, 0d);
        zombie.getPersistentDataContainer().set(new NamespacedKey(StaticPrisons.getInstance(), "value"), PersistentDataType.STRING, "0");
        zombie.getPersistentDataContainer().set(new NamespacedKey(StaticPrisons.getInstance(), "lastUpdated"), PersistentDataType.LONG, Instant.now().getEpochSecond());
    }
    public static BaseRobot createRobotFromEntity(Zombie zombie) {
        BaseRobot robot = new BaseRobot() {};
        if (!zombie.getPersistentDataContainer().has(new NamespacedKey(StaticPrisons.getInstance(), "isRobot"), PersistentDataType.INTEGER)) return null; //Not a robot
        robot.zombie = zombie;
        robot.PDC = zombie.getPersistentDataContainer();
        return robot;
    }
    public void setLevel(int amount) {
        PDC.set(new NamespacedKey(StaticPrisons.getInstance(), "level"), PersistentDataType.INTEGER, Math.min(5, amount));
    }
    public int getLevel() {
        return PDC.get(new NamespacedKey(StaticPrisons.getInstance(), "level"), PersistentDataType.INTEGER);
    }
    public String getType() {
        return PDC.get(new NamespacedKey(StaticPrisons.getInstance(), "type"), PersistentDataType.STRING);
    }
    public void setOil(double amount) {
        PDC.set(new NamespacedKey(StaticPrisons.getInstance(), "oil"), PersistentDataType.DOUBLE, amount);
    }
    public double getOil() {
        return PDC.get(new NamespacedKey(StaticPrisons.getInstance(), "oil"), PersistentDataType.DOUBLE);
    }
    ...
    public void setValue(BigInteger amount) {
        PDC.set(new NamespacedKey(StaticPrisons.getInstance(), "value"), PersistentDataType.STRING, amount.toString());
    }
    public BigInteger getValue() {
        return new BigInteger(PDC.get(new NamespacedKey(StaticPrisons.getInstance(), "value"), PersistentDataType.STRING));
    }
    ...
    public long getSecondsSinceLastUpdated() {
        return Instant.now().getEpochSecond() - zombie.getPersistentDataContainer().get(new NamespacedKey(StaticPrisons.getInstance(), "lastUpdated"), PersistentDataType.LONG);
    }
}
